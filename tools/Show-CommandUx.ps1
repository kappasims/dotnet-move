...
<#
.SYNOPSIS
    Render a human-readable transcript of DotnetMove's command-line UX across representative
...
Import-Module ([System.IO.Path]::Combine($repoRoot, 'src', 'DotnetMove.Core', 'DotnetMove.Core.psd1')) -Force
...
Add-Line "# DotnetMove command-line UX flow"
Add-Line
...
try {
    ...
    Add-Scenario -Title 'Capability probe' `
        ...
        -CommandText 'Get-DotnetMoveCapability' `
        -Command { Get-DotnetMoveCapability }
    ...
    Add-Scenario -Title 'Preview a project move (-WhatIf)' `
        ...
        -CommandText 'Move-Dotnet -Path ./src/Lib/Lib.csproj -Destination ./libs/Lib -WhatIf' `
        -Command {
            Move-Dotnet -Path (Join-Path $r1 (Join-Path 'src' (Join-Path 'Lib' 'Lib.csproj'))) `
    ...
    Add-Scenario -Title 'Perform the move' `
        ...
        -CommandText 'Move-Dotnet -Path ./src/Lib/Lib.csproj -Destination ./libs/Lib' `
        -Command {
            Move-Dotnet -Path (Join-Path $r2 (Join-Path 'src' (Join-Path 'Lib' 'Lib.csproj'))) `
    ...
    Add-Scenario -Title 'Error: project does not exist' `
        ...
        -CommandText 'Move-Dotnet -Path ./nope/Ghost.csproj -Destination ./libs/Ghost' `
        -Command {
            Move-Dotnet -Path (Join-Path (New-Sandbox) 'Ghost.csproj') -Destination 'X:/libs/Ghost' -ErrorAction Continue
        }
    ...
    Add-Scenario -Title 'Error: destination inside the source' `
        ...
        -CommandText 'Move-Dotnet -Path ./src/Lib/Lib.csproj -Destination ./src/Lib/nested' `
        -Command {
            Move-Dotnet -Path (Join-Path $r3 (Join-Path 'src' (Join-Path 'Lib' 'Lib.csproj'))) `
    ...
    Add-Scenario -Title 'Repair: report dangling entries' `
        -Narrative 'Someone moved Lib by hand; this shows what DotnetMove found, read-only.' `
