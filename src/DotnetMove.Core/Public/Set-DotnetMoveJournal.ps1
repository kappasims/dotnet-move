function Set-DotnetMoveJournal {
    <#
    .SYNOPSIS
    ...
    if ($Global) {
        ...
        $gitArgs = @('config', '--global', 'dotnetmove.journal', $value)
    } else {
        ...
        $gitArgs = @('-C', $repoFull, 'config', 'dotnetmove.journal', $value)
    }

    if (-not $PSCmdlet.ShouldProcess($scope, "Set dotnetmove.journal = $value")) { return }

    & git @gitArgs 2>$null
    if ($LASTEXITCODE -ne 0) {
        $PSCmdlet.WriteError([System.Management.Automation.ErrorRecord]::new(
                [System.InvalidOperationException]::new("Could not write $scope. Is git installed and (for local scope) is this a git repository? With no git, set `$env:DOTNETMOVE_JOURNAL instead."),
